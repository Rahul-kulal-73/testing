...
const VenueBookingSystem = () => {
  ...
  const [bookings, setBookings] = useState(() => {
    ...
    return {
      // --- More Past Bookings Data ---
      '2025-09-15': { 'Hall 1': { 'Full Day': 'Art Exhibition' } },
      '2025-09-20': { 'Hall 2': { 'Morning': 'Yoga Workshop', 'Afternoon': 'Meditation Session' } },
      '2025-09-22': { 'Hall 1': { 'Evening': 'Book Club Meet' } },
      '2025-09-25': { 'Hall 2': { 'Full Day': 'Community Fair' } },
      '2025-09-30': { 'Hall 1': { 'Afternoon': 'Press Briefing' }, 'Hall 2': { 'Evening': 'Gala Dinner' } },


      // --- Past Bookings Data ---
      '2025-09-28': { 'Hall 1': { 'Full Day': 'Charity Gala' } },
      '2025-10-01': { 'Hall 1': { 'Full Day': 'Government Summit'}, 'Hall 2': { 'Morning': 'Business Seminar', 'Afternoon': 'Product Launch' } },
      '2025-10-04': {
        'Hall 1': { 'Evening': 'Reunion Party' },
        'Hall 2': { 'Full Day': 'Music Concert' }
      },
      // --- Future Bookings Data ---
  ...
  const calculatedPrice = useMemo(() => {
    ...
    return pricing.slotPrices[selectedSlot.name] || 0;
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const baseClasses = `h-16 md:h-20 relative flex flex-col transition-all duration-200 ${!isPast ? 'cursor-pointer' : 'bg-gray-100 text-gray-400 opacity-75'}`;
      const borderClasses = isSelected ? 'ring-2 ring-blue-500 shadow-lg z-10' : isToday ? 'ring-2 ring-green-500' : '';

      if (calendarView === 'Both') {
        const bgClass = isPast ? '' : 'bg-white hover:bg-gray-50';
        days.push(
          <div key={day} onClick={() => !isPast && handleDateSelect(day)} className={`${baseClasses} p-2 ${borderClasses} ${bgClass}`}>
        ...
        days.push(
          <div key={day} onClick={() => !isPast && handleDateSelect(day)} className={`${baseClasses} p-2 ${bgClass} ${borderClasses}`}>
            ...
            <div>
              {isFullDay && <div className="text-right text-[9px] font-bold text-red-700">Full Day</div>}
              {isPartial && (
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {slots.map(slot => {
                  ...
                  const priceForSlot = pricing.slotPrices[slot.name];
                  return (
                    <button
                      ...
                      {isThisSlotBooked ? (
                        ...
                        <div className="text-center leading-tight">
                          <div className="flex items-center justify-center gap-2">
                             {slot.icon}
                             <span className="text-xs sm:text-sm font-semibold">{slot.name}</span>
                          </div>
                          <span className="text-xs font-bold text-green-700 mt-1 block">
                            ₹{priceForSlot.toLocaleString('en-IN')}
                          </span>
                        </div>
                      )}
                    </button>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex items-center justify-between mb-4">
              <button
                ...
                className="px-3 py-2 flex items-center gap-2 rounded-lg bg-white hover:bg-gray-100 border transition text-sm font-medium text-gray-600">
